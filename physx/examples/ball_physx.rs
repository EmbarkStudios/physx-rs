...
//use physx::prelude::*;
use physx::{
    geometry::{PxSphereGeometry, SphereGeometry},
    math::{PxTransform, PxVec3},
    physics::PhysicsFoundation,
    rigid_actor::RigidActor,
    rigid_body::RigidBody,
    scene::{SceneDescriptor, SceneFlag},
};
...
fn main() -> Result<(), ()> {
    ...
    /* This is the more involved intialization, including setting the allocator.
    use physx::{
        physics::PhysicsFoundationBuilder,
        foundation::{Foundation, DefaultAllocator, TrackingAllocator},
    };
    let mut physics_foundation = PhysicsFoundationBuilder::default()
    .build::<(), ()>(Foundation::new::<TrackingAllocator>().unwrap()).unwrap();
    */
    ...
    let ground_plane = physics
        .create_plane(PxVec3::new(0.0, 1.0, 0.0), 0.0, material.as_mut(), 0)
        .unwrap();
