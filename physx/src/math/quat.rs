...
impl PxQuat {
    ...
    pub fn dot(&self, other: &PxQuat) -> f32 {
        unsafe {
            PxQuat_dot(self.as_ptr(), other.as_ptr())
        }
    }

    pub fn get_identity_angle(&self) -> f32 {
        unsafe {
            PxQuat_getAngle(self.as_ptr())
        }
    }

    pub fn get_angle(&self, other: &PxQuat) -> f32 {
        unsafe {
            PxQuat_getAngle_1(self.as_ptr(), other.as_ptr())
        }
    }

    pub fn get_basis_vector_x(&self) -> PxVec3 {
        unsafe {
            PxQuat_getBasisVector0(self.as_ptr()).into()
        }
    }

    pub fn get_basis_vector_y(&self) -> PxVec3 {
        unsafe {
            PxQuat_getBasisVector1(self.as_ptr()).into()
        }
    }

    pub fn get_basis_vector_z(&self) -> PxVec3 {
        unsafe {
            PxQuat_getBasisVector2(self.as_ptr()).into()
        }
    }

    pub fn get_conjugate(&self) -> PxQuat {
        unsafe {
            PxQuat_getConjugate(self.as_ptr()).into()
        }
    }

    pub fn get_imaginary_part(&self) -> PxVec3 {
        unsafe {
            PxQuat_getImaginaryPart(self.as_ptr()).into()
        }
    }

    pub fn get_normalized(&self) -> PxQuat {
        unsafe {
            PxQuat_getNormalized(self.as_ptr()).into()
        }
    }

    pub fn is_finite(&self) -> bool {
        unsafe {
            PxQuat_isFinite(self.as_ptr())
        }
    }

    pub fn is_identity(&self) -> bool {
        unsafe {
            PxQuat_isIdentity(self.as_ptr())
        }
    }

    pub fn is_sane(&self) -> bool {
        unsafe {
            PxQuat_isSane(self.as_ptr())
        }
    }

    pub fn is_unit(&self) -> bool {
        unsafe {
            PxQuat_isUnit(self.as_ptr())
        }
    }

    pub fn magnitude(&self) -> f32 {
        unsafe {
            PxQuat_magnitude(self.as_ptr())
        }
    }

    pub fn magnitude_squared(&self) -> f32 {
        unsafe {
            PxQuat_magnitudeSquared(self.as_ptr())
        }
    }
    ...
    pub fn normalize(&mut self) -> f32 {
        unsafe {
            PxQuat_normalize_mut(self.as_mut_ptr())
        }
    }
    ...
    pub fn rotate(&self, vector: &PxVec3) -> PxVec3 {
        unsafe {
            PxQuat_rotate(self.as_ptr(), vector.as_ptr()).into()
        }
    }

    pub fn rotate_inverse(&self, vector: &PxVec3) -> PxVec3 {
        unsafe {
            PxQuat_rotateInv(self.as_ptr(), vector.as_ptr()).into()
        }
    }
    ...
    pub fn from_angle_axis(angle: f32, axis: PxVec3) -> PxQuat {
        unsafe {
            PxQuat_new_4(angle, axis.as_ptr()).into()
        }
    }
}
