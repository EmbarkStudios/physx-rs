...
impl Default for PxTransform {
    fn default() -> Self {
        unsafe { Self{obj: PxTransform_new_2(0)} }
    }
}
...
impl PxTransform {
    ...
    pub fn from_rotation(rotation: &PxQuat) -> PxTransform {
        unsafe {
            PxTransform_new_3(rotation.as_ptr()).into()
        }
    }

    pub fn from_translation_rotation(translation: &PxVec3, rotation: &PxQuat) -> PxTransform {
        unsafe {
            PxTransform_new_5(translation.as_ptr(), rotation.as_ptr()).into()
        }
    }

    pub fn get_inverse(&self) -> PxTransform {
        unsafe {
            PxTransform_getInverse(self.as_ptr()).into()
        }
    }

    pub fn get_normalized(&self) -> PxTransform {
        unsafe {
            PxTransform_getNormalized(self.as_ptr()).into()
        }
    }

    pub fn is_finite(&self) -> bool {
        unsafe {
            PxTransform_isFinite(self.as_ptr())
        }
    }

    pub fn is_sane(&self) -> bool {
        unsafe {
            PxTransform_isSane(self.as_ptr())
        }
    }

    pub fn is_valid(&self) -> bool {
        unsafe {
            PxTransform_isValid(self.as_ptr())
        }
    }

    pub fn rotate(&self, vector: &PxVec3) -> PxVec3 {
        unsafe {
            PxTransform_rotate(self.as_ptr(), vector.as_ptr()).into()
        }
    }

    pub fn rotate_inv(&self, vector: &PxVec3) -> PxVec3 {
        unsafe {
            PxTransform_rotateInv(self.as_ptr(), vector.as_ptr()).into()
        }
    }

    pub fn transform(&self, other: &PxTransform) -> PxTransform {
        unsafe {
            PxTransform_transform_1(self.as_ptr(), other.as_ptr()).into()
        }
    }

    pub fn transform_inv(&self, other: &PxTransform) -> PxTransform {
        unsafe {
            PxTransform_transformInv_1(self.as_ptr(), other.as_ptr()).into()
        }
    }

    pub fn transform_vector(&self, vector: &PxVec3) -> PxVec3 {
        unsafe {
            PxTransform_transform(self.as_ptr(), vector.as_ptr()).into()
        }
    }

    pub fn transform_vector_inv(&self, vector: &PxVec3) -> PxVec3 {
        unsafe {
            PxTransform_transformInv(self.as_ptr(), vector.as_ptr()).into()
        }
    }

    pub fn transform_plane(&self, plane: &PxPlane) -> PxPlane {
        unsafe {
            PxTransform_transform_2(self.as_ptr(), plane)
        }
    }

    pub fn transform_plane_inv(&self, plane: &PxPlane) -> PxPlane {
        unsafe {
            PxTransform_inverseTransform(self.as_ptr(), plane)
        }
    }
}
