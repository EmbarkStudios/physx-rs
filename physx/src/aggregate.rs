...
unsafe impl<P, L, S, D, H, M> Class<P> for Aggregate<L, S, D, H, M> where physx_sys::PxAggregate: Class<P> {
...
impl<L, S, D, H, M> Aggregate<L, S, D, H, M> {
    ...
    pub fn remove_articulation(&mut self, articulation: &mut impl ArticulationBase<L, H, M>) -> bool {
...
impl<L, S, D, H, M> Drop for Aggregate<L, S, D, H, M> {
    fn drop(&mut self) {
        // TODO should this call drop on the articulations or actors that are part of it?
        unsafe {
